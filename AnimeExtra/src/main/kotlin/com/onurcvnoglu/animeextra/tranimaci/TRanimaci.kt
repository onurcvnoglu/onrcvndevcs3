...
package com.onurcvnoglu.animeextra.tranimaci
...
class TRanimaci : MainAPI() {
    ...
    private fun Element.toMainPageResult(): SearchResponse? = with(api) {
        val title     = this@toMainPageResult.selectFirst("a")?.text() ?: return@with null
        val href      = fixUrlNull(this@toMainPageResult.selectFirst("a")?.attr("href")) ?: return@with null
        val posterUrl = fixUrlNull(this@toMainPageResult.selectFirst("div.limit img")?.attr("src"))

        return@with newAnimeSearchResponse(title, href, TvType.Anime) { this.posterUrl = posterUrl }
    }
    ...
    override suspend fun search(query: String): List<SearchResponse>? = with(api) {
        ...
        return@with document.select("article.bs div.bsx").mapNotNull { it.toMainPageResult() }
    }
    ...
    override suspend fun load(url: String): LoadResponse? = with(api) {
        ...
        val title       = document.selectFirst("h1")?.text()?.trim() ?: return@with null
        ...
        return@with newTvSeriesLoadResponse(title, url, TvType.TvSeries, episodeses) {
    ...
    override suspend fun loadLinks(
        ...
        for (i in 0 until videoSourceArray.length()) {
            ...
            for (j in 0 until mp4Array.length()) {
                ...
                callback.invoke(
                    newExtractorLink(
                        source = this@TRanimaci.name,
                        name = "${this@TRanimaci.name} - ${quality}p",
                        url = videoUrl,
